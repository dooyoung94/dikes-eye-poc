...
def current_context() -> dict:
    return {
        "date_or_day": str(st.session_state.get("ctx_day", "")).strip(),
        "time": str(st.session_state.get("ctx_time", "")).strip(),
        "purpose": str(st.session_state.get("ctx_purpose", "")).strip(),
        "preference": str(st.session_state.get("ctx_preference", "")).strip(),
    }
...
def context_items(context: dict, kind: str) -> list[tuple[str, str]]:
    ...
    return [(label, str(value).strip()) for label, value in candidates if str(value).strip()]


def render_context_chips(context: dict, kind: str, title: str = "내 조건") -> None:
    items = context_items(context, kind)
    if not items:
        st.markdown(
            f"<div class='context-panel'><div class='context-title'>🎯 {title}</div><div class='context-empty'>질문에서 별도 조건을 찾지 못했어요. 필요하면 아래에서 조건을 추가할 수 있습니다.</div></div>",
    ...
    st.markdown(
        f"<div class='context-panel'><div class='context-title'>🎯 {title}</div><div class='context-chips'>{chips}</div></div>",
        unsafe_allow_html=True,
    )
...
.context-chip span {color:#7d818a;}
.context-chip strong {color:#252a31;}
...
<div class="hero">
  ...
  <div class="hero-sub">질문에서 사용자의 상황을 먼저 읽고, 그 조건에 맞는 Evidence를 더 넓게 찾아 비교합니다. 같은 대상이라도 조건이 달라지면 검색 결과와 최종 판단도 달라집니다.</div>
  <div class="hero-chips"><span class="hero-chip">Context · 내 조건</span><span class="hero-chip">Strength · 반복 강점</span><span class="hero-chip">Rashomon · 찬반 비교</span><span class="hero-chip">Wald · 놓친 신호</span></div>
</div>
""",
    unsafe_allow_html=True,
)
...
with st.container(border=True):
    ...
    st.markdown('<div class="section-sub">대상과 상황을 한 문장으로 적어주세요. Dike가 대상·요일·시간·목적·중요조건을 먼저 읽어냅니다.</div>', unsafe_allow_html=True)
...
if intent:
    ...
    with st.container(border=True):
        ...
        render_context_chips(current_context(), kind, "질문에서 읽은 내 조건")
    ...
    if kind == "product" and not st.session_state.selected_target:
        with st.form("product_confirm_form"):
            ...
            product_confirmed = st.form_submit_button("이 제품이 맞아요", type="primary", use_container_width=True)
...
if selected and not st.session_state.analysis:
    ...
    st.markdown('<div class="section-sub">질문에서 읽은 조건을 그대로 사용합니다. 잘못 읽은 부분이 있을 때만 수정하면 됩니다.</div>', unsafe_allow_html=True)
    render_context_chips(current_context(), selected["kind"], "분석에 사용할 내 조건")
    ...
    with st.expander("조건이 다르면 여기서 수정"):
        ...
        st.caption("수정한 값은 바로 아래 분석 버튼에 반영됩니다.")

    if st.button("이 조건으로 판단하기", type="primary", use_container_width=True):
        context = current_context()
        try:
            with st.spinner("내 조건에 맞는 후기와 장단점, 리뷰 밖 신호를 더 넓게 수집하고 있어요..."):
...
if st.session_state.analysis and st.session_state.user_report:
    ...
    render_context_chips(result_context, a.get("kind", "restaurant"), "이번 판단에 사용한 내 조건")
    ...
    with s3:
        st.markdown(f"<div class='stat-card'><div class='stat-label'>검토 Evidence</div><div class='stat-value'>{total_count}건</div><div class='stat-note'>일반/조건 {visible_count} + 이탈 {hidden_count}</div></div>", unsafe_allow_html=True)

    context_visible_count = sum(1 for row in a.get("rows", []) if row.get("retrieval_scope") == "user_context")
    preference_visible_count = sum(1 for row in a.get("rows", []) if row.get("retrieval_scope") == "preference")
    with st.container(border=True):
        st.markdown("**🔍 이번 분석에서 실제로 확보한 범위**")
        q1,q2,q3 = st.columns(3)
        q1.metric("전체 Visible", f"{visible_count}건")
        q2.metric("내 조건 검색", f"{context_visible_count}건")
        q3.metric("핵심 선호 검색", f"{preference_visible_count}건")
        st.caption("같은 문서가 여러 검색 관점에서 잡힌 경우 중복 제거 후 한 번만 계산합니다.")
    ...
    if positive_aspects:
        for item in positive_aspects[:3]:
            ...
            with st.container(border=True):
                ...
                st.caption(f"긍정 {pos}건 / 부정 {neg}건으로, 반복적으로 확인되는 강점으로 반영했어요.")
    else:
    ...
    if conflicts:
        ...
        with st.container(border=True):
            ...
            st.caption("좋다는 의견과 아쉽다는 의견이 함께 있어, 평균값만으로 판단하기 어려운 부분이에요.")
    ...
    if rca_point:
        ...
        with st.container(border=True):
            ...
            if diff > 0:
                st.caption("내 조건에서는 부정 의견이 전체보다 더 자주 나타났어요. 이 부분은 실제 선택에서 주의할 필요가 있습니다.")
            elif diff < 0:
                st.caption("내 조건에서는 부정 의견이 전체보다 적었어요. 이 조건은 오히려 긍정적으로 반영됩니다.")
            else:
    ...
    if signal_counts:
        with st.container(border=True):
            ...
            st.caption("실제 발생률이 아니라 검색된 이탈·실패 신호의 건수라서 위험 보정은 제한적으로만 적용합니다.")
...
st.divider()
st.caption("Dike's Eye · 질문에서 조건을 읽고, 그 조건에 맞는 Evidence를 넓게 수집해 선택을 돕는 Decision Agent")