...
.hero{{position:relative;overflow:hidden;border-radius:32px;padding:2.55rem 3.2rem 2.15rem;text-align:center;color:#fff7e7;background:linear-gradient(145deg,#0b1a28,#153148 55%,#0b1a28);box-shadow:0 26px 60px rgba(24,30,34,.30);isolation:isolate}}
...
.st-key-question_panel,.st-key-intent_panel,.st-key-place_confirm_panel,.st-key-report_panel{{background:linear-gradient(rgba(255,252,244,.995),rgba(255,252,244,.995)),url('{PARCHMENT}') center/cover!important;border:1px solid #d7bd8b!important;border-radius:20px!important;box-shadow:0 14px 34px rgba(62,43,23,.16)!important;padding:1rem!important;color:#241d15!important}}
.st-key-place_confirm_panel{{margin:.7rem 0 1rem!important}}
.st-key-report_panel{{margin-top:1rem!important;padding:1.15rem!important}}
.st-key-place_confirm_panel p,.st-key-place_confirm_panel span,.st-key-place_confirm_panel label,.st-key-report_panel p,.st-key-report_panel span,.st-key-report_panel label,.st-key-report_panel strong{{color:#2b251d!important;opacity:1!important;text-shadow:none!important}}
.st-key-report_panel .section-sub{{color:#4f4b44!important}}
div[data-testid="stVerticalBlockBorderWrapper"]{{background:#fffaf1!important;border:1px solid #d7bd8b!important;border-radius:20px!important;box-shadow:0 14px 34px rgba(62,43,23,.12)!important;padding:.35rem!important;backdrop-filter:none!important}}
div[data-testid="stVerticalBlockBorderWrapper"]>div{{background:transparent!important}}
.st-key-place_confirm_panel [data-testid="stRadio"],.st-key-place_confirm_panel [role="radiogroup"]{{background:#fffdf8!important;border:1px solid #decba5!important;border-radius:14px!important;padding:.7rem .8rem!important}}
.st-key-place_confirm_panel [data-testid="stRadio"] label{{color:#201d18!important;font-weight:650!important}}
div[data-testid="stForm"]{{background:linear-gradient(rgba(255,250,240,.99),rgba(255,250,240,.99)),url('{INPUT_TEXTURE}') center/cover!important;border:1px solid #d5b97f!important;border-radius:16px!important;padding:.85rem!important;box-shadow:0 7px 18px rgba(64,47,27,.09)!important}}
.section-title{{display:inline-block;font-size:1.2rem;font-weight:850;letter-spacing:-.025em;margin:.65rem 0 .28rem;padding:.42rem .68rem;color:#241d15;background:rgba(255,248,234,.98);border:1px solid rgba(201,168,94,.55);border-radius:11px;box-shadow:0 5px 14px rgba(70,49,25,.10);text-shadow:none}}
.section-sub{{font-size:.84rem;line-height:1.55;color:#4f4b44;margin-bottom:.7rem}}
.result,.condition-card,.balance,.verdict{{background:linear-gradient(rgba(255,252,244,.99),rgba(255,252,244,.99)),url('{PARCHMENT}') center/cover;border:1px solid #d3b77e;border-radius:18px;padding:1rem;box-shadow:0 10px 25px rgba(68,49,28,.11)}}
...
.context{{background:linear-gradient(rgba(255,250,239,.995),rgba(255,250,239,.995)),url('{INPUT_TEXTURE}') center/cover;border:1px solid #d5b97f;border-radius:16px;padding:.78rem;margin:.65rem 0;box-shadow:0 6px 18px rgba(92,70,37,.09)}}
.context-title{{font-size:.76rem;font-weight:850;color:#71572f;margin-bottom:.45rem}}.chips{{display:flex;gap:.35rem;flex-wrap:wrap}}.chip{{display:inline-flex;gap:.28rem;align-items:center;background:#fffdf8;border:1px solid #decba5;border-radius:999px;padding:.32rem .54rem;font-size:.72rem}}.chip small{{color:#686861}}.chip b{{color:#28251f}}.chip.condition{{border-color:#c69a3f;background:#fff0cc}}
...
[data-testid="stMetric"]{{background:#fffaf1;border:1px solid #dcc79f;padding:.52rem .58rem;border-radius:12px}}[data-testid="stMetricValue"]{{font-size:1.15rem;font-weight:850}}
.stButton>button,[data-testid="stFormSubmitButton"]>button{{border-radius:12px;min-height:2.8rem;font-weight:800;background:linear-gradient(135deg,#11283c,#173b55)!important;color:#f0d397!important;border:1px solid #d4af37!important;box-shadow:0 7px 18px rgba(22,40,55,.18)}}
.stTextInput input{{border-radius:12px!important;background:#fffdf8!important;color:#272923!important;border:1px solid #d9c08e!important}}
[data-testid="stExpander"]{{border:1px solid #d5c09a;border-radius:14px;background:#fffaf2!important;color:#2b251d!important}}
[data-testid="stCaptionContainer"],.stCaption,p[data-testid="stCaptionContainer"]{{color:#5a554c!important}}
div[data-testid="stMarkdownContainer"] p{{text-shadow:none}}
@media(max-width:700px){{[data-testid="stAppViewContainer"]{{background:url('{BG_MOBILE}') center top/auto 100vh fixed no-repeat;background-color:#e7d8c0}}.block-container{{padding:.5rem .65rem 4rem}}.hero{{border-radius:20px;padding:1.35rem .9rem 1.18rem}}.hero-scale{{width:52px;height:52px}}.eyebrow{{font-size:.54rem;letter-spacing:.11em}}.hero h1{{font-size:1.55rem;line-height:1.31;margin:.48rem 0}}.hero p{{font-size:.76rem;line-height:1.6}}.hero-tags{{gap:.1rem;margin-top:.45rem}}.hero-tags span{{font-size:.55rem;padding:.2rem .32rem}}.intro-title{{font-size:.9rem}}.journey-grid{{grid-template-columns:repeat(2,1fr);gap:.42rem}}.journey-card{{min-height:155px;padding:.72rem .4rem}}.journey-card img{{width:48px;height:48px}}.journey-card b{{font-size:.73rem}}.journey-card p{{font-size:.62rem}}.section-title{{font-size:1rem;padding:.36rem .55rem}}.section-sub{{font-size:.77rem}}.st-key-question_panel,.st-key-intent_panel,.st-key-place_confirm_panel,.st-key-report_panel{{padding:.75rem!important;border-radius:16px!important}}div[data-testid="stVerticalBlockBorderWrapper"]{{border-radius:16px!important;padding:.18rem!important}}div[data-testid="stForm"]{{border-radius:13px!important;padding:.65rem!important}}.result,.condition-card,.balance,.verdict{{padding:.8rem;border-radius:15px}}.condition-head{{display:block}}.badge{{display:inline-block;margin-top:.28rem}}.signals{{grid-template-columns:1fr}}.balance-grid{{grid-template-columns:1fr 36px 1fr;gap:.28rem}}[data-testid="column"]{{min-width:0!important}}}}
</style>
...
if intent:
    ...
    with st.container(border=True, key="intent_panel"):
    ...
    if kind == "restaurant" and not st.session_state.selected_target:
        ...
        if st.session_state.get("candidates"):
            ...
            with st.container(key="place_confirm_panel"):
                st.markdown("<div class='section-title'>이 장소가 맞나요?</div>", unsafe_allow_html=True)
                idx = st.radio("장소 후보", range(len(labels)), format_func=lambda i: labels[i], label_visibility="collapsed")
...
if st.session_state.analysis and st.session_state.user_report:
    ...
    with st.container(key="report_panel"):
        ...
        if conflicts:
            ...
            for i, x in enumerate(conflicts[:3]):
                with st.container(border=True, key=f"conflict_{i}"):
        ...
        st.markdown("<div class='section-title'>Ⅵ. Dike의 최종 판단 · Solomon Choice</div>",unsafe_allow_html=True)
