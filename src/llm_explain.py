...
def fallback_explanation(analysis: dict[str, Any]) -> dict[str, Any]:
    ...
    reasons = []
    if confidence < 55:
        reasons.append("현재 Evidence의 양·출처·사용자 조건 일치도가 충분히 높지 않아 판단 신뢰도가 제한적입니다.")
    else:
        reasons.append("현재 수집된 Evidence의 양과 최신성, 사용자 조건 일치도를 함께 반영했습니다.")
    ...
    if wald_risk > 0.35:
        reasons.append("예약 실패·웨이팅 포기 등 보이지 않는 이탈 신호도 함께 고려했습니다.")
    else:
        reasons.append("Wald 관점의 이탈 신호는 현재 Evidence에서 강하게 나타나지 않았습니다.")

    return {
        "headline": f"{label} · 적합도 {score}/100",
        "answer": f"현재 조건 기준으로는 {label} 판단입니다. 판단 신뢰도는 {confidence}%이며, 점수는 LLM이 아니라 deterministic scoring engine이 계산했습니다.",
        "reasons": reasons[:3],
        "risks": [
            "Wald 신호는 전체 방문자 대비 실제 실패율이 아니라 선택편향 위험 신호입니다.",
...
Wald는 실제 실패율이 아니라 보이지 않는 이탈/선택편향 위험 신호로 표현하라.
사용자 상황에 맞춰 한국어로 간결하게 답하라.
